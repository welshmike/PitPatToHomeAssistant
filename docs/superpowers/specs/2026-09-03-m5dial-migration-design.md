...
1. **The swatch ring is rotated half a step.** `LightLayout::kSwatchStartDeg = 22.5` is the angle of swatch 0 clockwise from 12 o'clock (radius still 74), so no swatch sits at 12 or 6 o'clock. Starting at 12 put swatch 0 under the page-dot row and swatch 4 on the small power glyph, whose hit discs overlapped: a tap at the bottom of the ring was ambiguous. Rotated, the nearest centres (swatches 3 and 4, at 157.5 and 202.5 degrees, ~(148,188) and ~(92,188)) are ~37 px from the glyph centre (120,212), past the `kSwatchHitR + kPowerGlyphHitR` = 32 px at which the discs would touch; a layout test sweeps the round face and asserts the two hit tests never both claim a point.
2. **The Colour page draws bare:** no card name and no page dots, only the ring, the centre disc and the small power glyph. The ring is unmistakable on its own and the rotation leaves no clean room for either.
3. **Tap priority on the Colour page is swatch before power glyph**, so a swatch wins outright regardless of what the geometry later becomes.
