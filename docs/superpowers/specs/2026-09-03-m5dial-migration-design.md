...
- **Boot card is Clock.** ~~No idle return~~ (idle return added by the 2026-09-08 amendment at the end of this section). **Backlight dims after 2 min and never turns off** (the OFF stage is removed); the first input on a dimmed screen only wakes it.
- **Clock card**: analogue face on the round display — 12 tick marks, hour/minute/second hands, small date (`Mon 3 Sep`) above the centre (moved from 6 o'clock by the §4.19 clock-layout amendment), redrawn once a second. Time from NTP over WiFi with the POSIX TZ string `TIMEZONE_TZ` in `config.h` (default Europe/London), and the Dial's BM8563 RTC is synced from NTP and read at boot so the clock is right before WiFi is up. When time is unknown (fresh device, no WiFi, empty RTC) the face shows `--:--`.
- Data cards (later) refresh on their own interval and show a small stale marker when the last fetch failed.
- **Amended 2026-09-08 (idle return, Mike):** after **2 min** without input (the same clock as the dim, `DialInput::DIM_AFTER_MS`), any desk card other than Clock — Treadmill, Flights, Office, Lamp, Calendar — returns to the **Clock** card, so a dimmed Dial always shows the clock and today's next meeting. Conditions: the belt is idle (the resolved screen is a desk card, so Connecting/Starting/Running/Paused are never preempted), no card menu or speed selector is open (they keep their own 8 s / 20 s timeouts), and no Flights auto-show or Calendar nudge is holding its card (`FlightsAutoShow::isShowing()`, `CalendarAutoShow::isShowing()`) — a 5 min nudge must outlive 2 min of inactivity. The return goes through `navigateToCard()`, so the light cards also land on their first page. Decision inputs: `DialInput::idleFor(nowMs, ms)`, natively tested.
...
- **Amended 2026-09-08 (logo fallback, Mike):** two hardware findings. (a) The Flights card sometimes showed a black rectangle in the white band with fragments of the previous frame: `render()` deferred the PNG decode when free heap was under 80 KB or the largest block under 48 KB, but the frame had already reserved the logo rectangle as `TRANSPARENT`, so the hole showed whatever was on the display before and stayed while the heap stayed low. Now `drawFlights()` reserves the rectangle only when the logo is already on screen or `DialUi::logoDecodeHeapOk()` passes at frame time; otherwise the operator/callsign text fallback is drawn, and the 250 ms frame fallback brings the logo in as soon as the heap allows. The first deferral per airline is logged at `log_w` with the heap figures. (b) A newly-seen airline showed text for ten minutes: HA fetches the logo from the internet on first sight, the Dial's first GET raced it and the 404 was remembered for `kLogoMissingTtlMs` = 10 min. Now 60 s. RemoteLog also forwards info lines containing `logo`, so the download/decode path is visible on `pacekeeper-dial/diag`.
- **Amended 2026-09-09 (raw logos via the relay, Mike):** after ~20 h of uptime the Dial's free heap had drifted from ~81 KB to ~70 KB and the 8 Sep heap gate was refusing every PNG decode (`logo TK deferred for heap (70216 free / 55284 largest)`), so logos showed as text all morning. The decode is removed from the Dial. The Mac-mini relay (`tools/calendar_feed/calendar_feed.py`, now `/logo/{IATA}.565` beside `/calendar.json`) fetches each airline's PNG once from `https://pics.avs.io/120/48/{IATA}.png`, scales it to 108 × 43, composites it on white inside a 120 × 48 canvas at (6, 2) — the geometry the Dial used to render — and serves raw **big-endian RGB565**, exactly 11,520 bytes, `application/octet-stream`, cached on disk under `~/Library/Application Support/pacekeeper-calendar/logos/` (config `LOGO_DIR`) and in memory; a 404 at the source is remembered for 10 min, transient failures answer 503. No token on this route (public airline marks). `FLIGHTS_LOGO_BASE_URL` defaults to `http://macmini.lan:8765`; `FlightsService` GETs `/logo/{IATA}.565`, **streams** the body through a 1 KB buffer (was an 8 KB whole-body buffer) into `/logos/{IATA}.tmp` on LittleFS and renames it to `/logos/{IATA}.565` once it is exactly `kLogoBytes` with the right Content-Type; the cache-hit check is the file length; leftover `.png`/`.tmp` files are swept at boot. `dialDrawAirlineLogo()` copies the file to the panel one 240-byte row at a time as `lgfx::swap565_t` — no pngle, no sprite, no malloc — so the heap gate, deferral log and 76 KB constant are gone and a cached logo is drawable at any heap level. HA's `pacekeeper_dial_airline_logos` automation and the `downloader` integration are no longer used by the Dial (left in place, documented as superseded). The 8 Sep 60 s negative cache and `logo` log forwarding stay. The relay answers a not-yet-cached logo with an immediate **503** and fetches it in the background (its first inline fetch made the Dial's 3 s GET time out); the Dial treats that 503 as "warming" and retries after **700 ms**, up to 6 times, before the normal 5 s → 60 s back-off resumes, so a new airline's logo appears about as fast as pics.avs.io answers. Also from this date RemoteLog forwards the loop's `Heap:` line (now every 60 s, was 15 s unforwarded) so the drift can be trended on `pacekeeper-dial/diag`.
...
- **Side button, belt idle:** *single click* opens the **card menu**; *hold* (M5Unified `wasHold`, 500 ms) jumps to the Treadmill card, closing any menu or selector; *double click* (`wasDoubleClicked`, added 2026-09-08 at Mike's request) jumps to the **Clock** card the same way, from any desk card or the open menu. On the belt screens each press of a double click is still an instant stop. Single click is decided after M5Unified's multi-click window (~350 ms); that latency is accepted on desk cards only.
...
- **On:** page dots at y 54 (Lamp: Brightness → Kelvin → Colour; Office: Brightness → Kelvin) *(order changed by §4.18 amendment: Brightness → Colour → Kelvin)*. Horizontal swipe changes page (no wrap). Every arrival on the card, and every switch-on, starts on Brightness. Small power glyph at (120,212) r 8: tap (hit r 14) → off. Touch-hold anywhere 1 s → off, with the existing red hold arc.
...
2. **The Colour page draws bare:** no card name, no page dots and no power glyph — only the ring and the centre disc (Mike, 2026-09-06: nothing under the picker). Leave it by swiping back *(since §4.18's scrub: the swipe must start inside the centre, within 70 px)* or via the 10 s page timeout; the glyph is not a tap target there.
...
- **Knob:** ±`HUE_STEP` = 15° per detent, wrapping at 360 (24 detents per revolution). Same 300 ms settle as every page: a fast turn sends one command.
...
- **Success (Phase K, Mike on hardware):** tapping the ring at 3 o'clock sends hue 90 and the lamp goes green; three clockwise detents from there send 135; the marker sits on the lamp's live hue after an HA-side change; the ring dims with the caption when the lamp is on a colour temperature and a tap brings colour back; Office (no colour) is unaffected.
- **Amended 2026-09-06 (Mike, after the first hardware try):** the knob step is **15°** per detent (24 per revolution), not 5°: three 5° steps in purple were indistinguishable ("the dial does not change the colour"). And the ring is **scrubbable**: a touch that begins inside the touch annulus is claimed by the ring — for as long as the finger stays down, each tick sets the edit hue to `hueAt(touchX, touchY)` (angle only; the finger may drift off the band), re-arming the same 300 ms settle, so the marker and centre disc follow the finger live and the lamp receives one command 300 ms after the finger pauses or lifts. A claimed touch produces no swipe, no long-press (no hold arc) and no tap; a touch that begins anywhere else behaves as before. `DialInput` therefore reports the finger position every tick (`touchHeld`, `touchX/Y`, `touchStartX/Y`) and offers `claimTouch()`; the tap path on the Colour page is subsumed by the scrub path (a tap is a one-tick scrub). Phase K's three-detent line becomes 90 → 135. As built: the claim is decided once on the touch-down tick (`DialEvents::touchBegan`) and the send policy lives in `LightCardState::scrub()` (first call selects; later calls only after ≥ 1° of movement from the gesture's last selection; page-idle refreshed), so a resting finger sends once and HA's echo cannot re-trigger a send.
- **Amended 2026-09-06 (Mike, second hardware try):** (a) **Page order** on the Lamp is Brightness → **Colour** → Kelvin (Office stays Brightness → Kelvin); the page sequence is a per-card list (`LightCardState::pageAt(i)`, `pageIndex()`), not the enum order, and the page dots, swipes and the 10 s fallback follow it. (b) **Tapping the centre disc on the Colour page leaves the page** (back to Brightness, accept beep), on the dim page too. Hit radius 34 around (120,120) — well inside the ring's touch band (inner radius 70), so it can never start a scrub.

## 4.19 Calendar card (added 2026-09-07, approved)

Decided with Mike 2026-09-07. The work calendar is Google Workspace and must never pass through Home Assistant; Google-hosted glue is acceptable. Google's device-code OAuth flow does not offer Calendar scopes, so the 3 Sep idea (device-code OAuth on the Dial) is dropped in favour of a Google Apps Script web app in Mike's own work account that hands the Dial a compact "next few events" JSON. Work only for now; the data model leaves room for a second source later.

- **Apps Script (Mike's work Google account).** `doGet(e)`: reject unless `e.parameter.k` equals a shared token; read `CalendarApp.getDefaultCalendar()` events from now to the end of tomorrow (script time zone Europe/London); drop events Mike has declined (`getMyStatus() == NO`); sort by start; return at most **5** as `{"t":<unix now>,"ev":[{"s":<start unix>,"e":<end unix>,"n":"<title, ≤ 40 chars>","a":0|1,"l":"<location or meeting host, ≤ 24 chars>"}]}` (`a` = all-day), `ContentService` JSON. Deployed as a web app, *Execute as me*, *Anyone with the link*; the token is the only secret. Script source and deploy steps live in `doc/CALENDAR_APPS_SCRIPT.md`; Mike pastes the `/exec` URL and token into `src/config.h` as `CALENDAR_URL` and `CALENDAR_TOKEN` (gitignored). The Calendar card and service compile only when `CALENDAR_URL` is defined.
- **Fetch (`CalendarService`, net task).** Modelled on the pre-§4.11 FlightsService: `WiFiClientSecure` + `HTTPClient`, `setFollowRedirects(HTTPC_STRICT_FOLLOW_REDIRECTS)` because `/exec` always answers 302 to `script.googleusercontent.com`; TLS trust via `setCACert()` with the concatenated PEMs of **GTS Root R1 and GTS Root R4** in `src/CalendarCerts.h` (both Google hosts chain to them; a `CALENDAR_TLS_INSECURE` define falls back to `setInsecure()` if Google ever re-roots). Timeouts 5 s connect / 8 s read. Heap guard as before: skip when free heap < 60 KB or largest block < 20 KB. Response cap 2 KB into a static buffer, parsed with ArduinoJson into `CalendarModel::Snapshot`. Poll every **5 min** while WiFi is up, plus an early refresh when a nudge is due within 60 s and the snapshot is older than 60 s. On failure back off 30 s → 1 → 2 → 4 → … capped at 30 min; a success resets it. One request in flight at a time; never from the loop task. Fetch outcome logged at `log_w` on failure (forwarded by RemoteLog), `log_i` on success. Snapshot handed to `DialUi` through the existing snapshot-store pattern (`SnapshotStore<CalendarModel::Snapshot>` written on the net task, read on the loop task).
- **Model (`CalendarModel`, pure, natively tested).** `Snapshot { bool valid; uint32_t fetchedAtEpoch; uint8_t count; Event ev[5]; }`, `Event { uint32_t start, end; bool allDay; char title[41]; char where[25]; }`. Helpers: `parse(json, out)`; `nextTimed(snapshot, nowEpoch)` = first non-all-day event with `end > now` (an event in progress counts); `countdownText(event, nowEpoch, buf)` → `in 25 min` / `in 2 h 05` / `now, 12 min left` / `starts now`; `isStale(snapshot, nowEpoch)` = `fetchedAt` older than 30 min or `!valid`. Times are UTC epochs from Google; local display uses `TimeService` (Europe/London).
- **Card (`CardId::CALENDAR`, `Screen::CALENDAR`).** Sixth card in the ring, after Lamp: Treadmill, Clock, Flights, Office, Lamp, Calendar. `CardMenu` becomes a ring of 6 (60° apart, same radius 88, same glyph size); a calendar glyph (page with two rings and a date bar) joins `DialGlyphs`. Face, drawn in `DialCalendarView.cpp` on the palette canvas: card name `Calendar` at the usual top position; the next timed event's **start time** (`14:30`, Font4) at y 62; its **title** centred around y 108 in Font4, wrapped to at most two lines of ~16 characters, ellipsis on overflow; the **countdown** (Font2, amber `PENDING` while in progress, `TEXT` otherwise) at y 150; then up to **three following** events, one Font2 line each at y 176/192/208, `14:30 Title…` in `DIM`; today's all-day events collapse into one `DIM` Font2 line at y 40 (`All day: Title` or `All day: 2 events`) and never nudge. **Empty**: `nothing more today` at the centre (Font4, `DIM`), with tomorrow's first event underneath as `Tomorrow 09:00 Title` when the payload has one. **Offline**: `no calendar` (Font4, `DIM`) plus `last update 47 min ago` when `isStale()`; `waiting for calendar` before the first fetch. (Without `CALENDAR_URL` the *service and the face* are compiled out, but `CardId::CALENDAR` is not: gating the enum would fork `CardRing`/`CardMenu` and their native suites, so such a build still shows a sixth "Calendar" entry in the menu and it resolves to the **Clock** face. No "not configured" face is needed — the card is simply unreachable as itself.) The knob does nothing here. Tap: dismisses an active nudge (below), otherwise nothing.
- **Nudge (`CalendarAutoShow`, pure, natively tested).** Same shape as `FlightsAutoShow`: `update(nowMs, nowEpoch, hasNext, nextStart, currentCard, beltIdle, dataValid)` → `SHOW_CALENDAR` when enabled, `beltIdle`, `currentCard == CLOCK`, data valid and fresh, and `nextStart - nowEpoch ≤ leadSec` for an event not yet dismissed; `RETURN_TO_CLOCK` when this machine showed the card, `nowEpoch ≥ nextStart + staySec`, and `currentCard` is still `CALENDAR`. `dismiss()` (a tap on the card during a nudge) records that event's start so it never re-nudges, and returns to Clock at once. Manual navigation elsewhere forgets the episode, as with Flights. A Flights auto-show in progress means `currentCard != CLOCK`, so the nudge waits for it to return (accepted, rare). Settings, persisted in NVS and exposed in HA like the other config entities: switch **Calendar nudge** (`cal_nudge`, default on), number **Calendar lead** (`cal_lead`, 1–30 min, default 5), number **Calendar stay** (`cal_stay`, 0–10 min, default 1). New `CmdType`s `SET_CALENDAR_NUDGE/LEAD/STAY` and matching `PubType`s.
- **Heap and coexistence.** One TLS session of ≤ 2 KB every 5 min is the load Plan 5's mitigations were designed for; the `esp_coex_preference_set(ESP_COEX_PREFER_BT)` bias stays. The Phase L checklist watches the `Heap:` line and the belt link across several fetches.
- **Not in scope.** Multiple or personal calendars, joining meetings, writing to the calendar, notifications on the belt screens.
- **Tests (native).** `CalendarModel`: parse (full payload, all-day flag, clipping, empty list, malformed JSON → invalid), `nextTimed` (skips all-day and finished events, in-progress event wins), `countdownText` (the four forms and the hour boundary), `isStale`. `CalendarAutoShow`: show at lead, no show when disabled / belt busy / not on Clock / stale, return after stay, dismiss blocks that event only, manual navigation forgets the episode, lead/stay setters. `CardMenu`: 6-item ring geometry and hit test. Existing suites untouched.
- **Success (Phase L, Mike on hardware).** Card appears after Lamp in the menu; within 10 s of boot the face shows the next real meeting with a correct London time and countdown; a fake meeting 6 min out nudges the Clock at 5 min and returns 1 min after the start; tap during the nudge returns at once and the same meeting does not nudge again; belt running suppresses the nudge; pulling the Apps Script deployment shows `no calendar` within 30 min; `Heap:` stays above 80 KB and the belt link survives ten fetches.
- **As built:** card name drawn at y 20 (not the Lights view's y 34) so it clears the all-day row at y 40; the two Font4 title lines sit at y 94/122 (centred on 108) to avoid descender clipping; every variable-length line is trimmed to the visible chord of the round face (`fitToRow`); `Tomorrow …` uses `CalendarModel::firstTimedOnLaterDay()` (skips all-day events); fetch forces HTTP/1.0 (`useHTTP10(true)`) because Apps Script's redirect target answers chunked and the raw stream is not de-chunked; per-hop timeouts are 4 s connect / 6 s read with a 12 s body-read budget, worst case ≈ 20–22 s, so the MQTT keepalive is raised from PubSubClient's 15 s default to 60 s; the net task stack is 16 KB (was 12 KB) for the verifying TLS handshake and logs its high-water mark after the first fetch attempt; the desk-card screen list is centralised in `DialUi::isDeskScreen()`; settings setters are `setCalendarLeadMin/StayMin`; the three HA settings exist on every build (not gated on `CALENDAR_URL`), as does `CardId::CALENDAR` and the six-item menu — a build without `CALENDAR_URL` shows the menu entry and lands on the Clock face (`DialUi::screenForCard()`); the face filters the feed by **local day** (`nextTimedToday()`, `allDayCountToday()`, `firstAllDayToday()`) because the payload runs to the end of tomorrow, so tomorrow’s meetings never appear as today’s and the `nothing more today` face is reachable, while the nudge keeps using the day-blind `nextTimed()` (a 09:00 meeting must still nudge from the Clock at 08:55); the nudge is also bounded from below — no `SHOW_CALENDAR` for an event already under way, which would otherwise flash the card and hand it straight back; `isStale()` treats a device clock behind the feed’s `t` as fresh rather than underflowing; native tests 354. `CalendarService` picks a plain `WiFiClient` or `WiFiClientSecure` from `CALENDAR_URL`'s scheme at compile time (both kept as function locals so only the https path ever touches mbedTLS's heap), and `CALENDAR_TOKEN` is optional — appended as `?k=` only when defined.
- **Amended 2026-09-07 (feed source):** Mike's Workspace only allows Apps Script web apps for "anyone in the company", which the Dial cannot be, and he does not want the account in Apple Calendar. The calendar's **secret iCal address** is available but is ~2 MB (the whole calendar with recurrence rules), too much to stream and expand on the device. So the feed becomes a **relay on the always-on Mac mini**: `tools/calendar_feed/calendar_feed.py` (Python 3, `icalendar` + `recurring-ical-events`) fetches the `.ics` every 5 min, expands recurrences for now → end of tomorrow in Europe/London, drops events Mike declined (matched by `MY_EMAIL`), and serves the unchanged compact JSON (`{"t","ev":[{"s","e","n","a","l"}]}`, ≤ 5 events) on the LAN at `http://<mac-mini-ip>:8765/calendar.json`. Configuration lives in `~/.config/pacekeeper/calendar.env` (`ICS_URL`, `MY_EMAIL`, optional `PORT`, optional `TOKEN`), never in the repo; a launchd agent keeps it running; `tools/calendar_feed/install.sh` sets up a venv, installs the two packages and loads the agent. The Dial's `CALENDAR_URL` is that `http://` address; `CalendarService` uses a plain `WiFiClient` for `http://` and keeps the TLS path (GTS roots) for `https://`; `CALENDAR_TOKEN` becomes optional (appended as `?k=` when defined; the relay checks it when `TOKEN` is set). The Apps Script and its deploy doc are removed; `doc/CALENDAR_FEED.md` replaces them. The 16 KB net-task stack and 60 s MQTT keepalive stay as headroom. Data path: Google → Mac mini → Dial; Home Assistant is not involved.
- **Amended 2026-09-07 (clock subline, Mike):** the Clock card shows **today's next timed meeting** as one Font2 line under the date, at y 186, `DIM` (amber `PENDING` while the meeting is in progress), formatted `HH:MM Title` and trimmed to the round bezel. Shown only when the calendar snapshot is valid and fresh and a timed event remains **today**; nothing for all-day events, nothing for tomorrow, nothing without `CALENDAR_URL`. The text is produced by `CalendarModel::clockLine(snapshot, nowEpoch, localDayOf, hhmm, buf, cap)` (pure, natively tested) and `drawClockCard()` takes it as an optional `const char* subline` plus `bool live`.
- **Amended 2026-09-07 (clock layout, Mike):** on the Clock card the **date moves above the centre** (y 72, Font2, `DIM`) and the next-meeting text sits below the centre on **two lines**: the **title** at y 168 (Font2, `DIM`) and the **time** `HH:MM` beneath it at y 186 (Font2, `DIM`, amber `PENDING` while the meeting is in progress). Both lines trimmed to the bezel; both absent when there is no timed meeting left today. `CalendarModel::clockLine()` therefore fills two buffers (title, time) instead of one. The "waiting for time" hint stays at the date's position.
- **Amended 2026-09-07 (refresh cadence and date size, Mike):** late invites and cancellations should reach the Dial within a couple of minutes, so the Dial polls the relay every **60 s** (`kPollMs`; plain LAN HTTP, ~430 B, negligible radio time) and the relay re-reads the 2 MB `.ics` every **120 s** (`REFRESH_S`; ~1.4 GB/day from Google, judged acceptable, 60 s was not). Worst-case latency Google → Dial ≈ 3 min plus Google's own feed lag. The Clock card's date is drawn in **Font4** (one size larger) at y 72. **Amended 2026-09-08:** Font4 (26 px) was a step too far; the date is now `fonts::DejaVu18` (18 px, proportional), one size between Font2 and Font4.
