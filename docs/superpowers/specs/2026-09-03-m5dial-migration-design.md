...
- **As built:** card name drawn at y 20 (not the Lights view's y 34) so it clears the all-day row at y 40; the two Font4 title lines sit at y 94/122 (centred on 108) to avoid descender clipping; every variable-length line is trimmed to the visible chord of the round face (`fitToRow`); `Tomorrow …` uses `CalendarModel::firstTimedOnLaterDay()` (skips all-day events); fetch forces HTTP/1.0 (`useHTTP10(true)`) because Apps Script's redirect target answers chunked and the raw stream is not de-chunked; per-hop timeouts are 4 s connect / 6 s read with a 12 s body-read budget, worst case ≈ 20–22 s, so the MQTT keepalive is raised from PubSubClient's 15 s default to 60 s; the net task stack is 16 KB (was 12 KB) for the verifying TLS handshake and logs its high-water mark after the first fetch attempt; the desk-card screen list is centralised in `DialUi::isDeskScreen()`; settings setters are `setCalendarLeadMin/StayMin`; the three HA settings exist on every build (not gated on `CALENDAR_URL`); native tests 344.
